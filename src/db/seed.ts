...
const generateAssetImages = (category: string, assetName: string, count: number = 3): string[] => {
    ...
    for (let i = 0; i < count; i++) {
        ...
        images.push(`https://placehold.co/800x600/${color}/${textColor}?text=${encodeURIComponent(text)}`);
    }
...
async function seedUsers() {
    ...
    users.push(
        ...
        // Logistics Staff
        {
            ...
            permissions: [
                'auth:*', 'users:read', 'companies:read', 'brands:read', 'warehouses:read', 'zones:read',
                'assets:*', 'collections:*', 'orders:read', 'orders:update', 'orders:add_time_windows',
                'pricing:review', 'pricing:approve_standard', 'pricing:adjust',
                'lifecycle:progress_status', 'lifecycle:receive_notifications',
    ...
    for (const company of seededData.companies.slice(0, 3)) { // Platform 1 companies
        users.push({
            platform_id: platform1.id,
            company_id: company.id,
    ...
    // Platform 2 users
    for (const company of seededData.companies.slice(3)) { // Platform 2 companies
        users.push({
            platform_id: platform2.id,
            company_id: company.id,
...
async function seedBrands() {
    ...
    for (const company of seededData.companies) {
        const brandNames = company.name === 'Diageo' 
    ...
    for (const emirate of emirates) {
        for (const tripType of tripTypes) {
            for (const vehicleType of vehicleTypes) {
                const baseRate = vehicleType === 'STANDARD' ? 500 : vehicleType === '7_TON' ? 800 : 1200;
...
async function seedServiceTypes() {
    ...
    const services = [
        ...
        { name: 'Crane Service', category: 'EQUIPMENT' as const, unit: 'day', default_rate: '1500.00', description: 'Mobile crane rental with operator' },
        ...
        { name: 'Custom Fabrication', category: 'RESKIN' as const, unit: 'unit', default_rate: null, description: 'Custom fabrication and branding (quote required)' },
        ...
        // Other services
        { name: 'Storage Fee', category: 'OTHER' as const, unit: 'day', default_rate: '50.00', description: 'Daily storage charge for extended periods' },
        { name: 'Rush Service', category: 'OTHER' as const, unit: 'trip', default_rate: '750.00', description: 'Expedited delivery/pickup' },
        { name: 'After Hours Service', category: 'OTHER' as const, unit: 'hour', default_rate: '180.00', description: 'Services outside business hours' },
...
async function seedAssets() {
    ...
    const assetTemplates = {
        'Furniture': [
            { name: 'Executive Round Table', description: 'Premium 6-seater round table', weight: 45, dims: {length: 150, width: 150, height: 75}, volume: 1.688 },
            { name: 'Chiavari Gold Chair', description: 'Classic gold chiavari chair', weight: 5.5, dims: {length: 40, width: 45, height: 90}, volume: 0.162 },
            { name: 'Velvet Lounge Sofa', description: '3-seater luxury velvet sofa', weight: 65, dims: {length: 210, width: 90, height: 85}, volume: 1.606 },
            { name: 'Bar Stool High', description: 'Modern metal bar stool', weight: 8, dims: {length: 45, width: 45, height: 110}, volume: 0.223 },
            { name: 'Cocktail Table Round', description: 'High-top cocktail table', weight: 18, dims: {length: 80, width: 80, height: 110}, volume: 0.704 },
            { name: 'Banquet Rectangle Table', description: '8-seater rectangular table', weight: 52, dims: {length: 240, width: 100, height: 75}, volume: 1.800 },
            { name: 'Ghost Chair Clear', description: 'Transparent acrylic chair', weight: 6, dims: {length: 46, width: 54, height: 94}, volume: 0.233 },
            { name: 'Luxury Ottoman', description: 'Velvet upholstered ottoman', weight: 12, dims: {length: 60, width: 60, height: 45}, volume: 0.162 },
            { name: 'Stage Platform 2x2m', description: 'Modular stage platform', weight: 85, dims: {length: 200, width: 200, height: 40}, volume: 1.600 },
            { name: 'DJ Booth Custom', description: 'Professional DJ booth setup', weight: 95, dims: {length: 180, width: 80, height: 110}, volume: 1.584 },
        ],
        'Glassware': [
            { name: 'Wine Glass Bordeaux', description: 'Premium crystal wine glass', weight: 0.25, dims: {length: 10, width: 10, height: 24}, volume: 0.002 },
            { name: 'Champagne Flute', description: 'Elegant champagne flute', weight: 0.22, dims: {length: 8, width: 8, height: 26}, volume: 0.002 },
            { name: 'Whisky Tumbler', description: 'Heavy base whisky glass', weight: 0.35, dims: {length: 9, width: 9, height: 10}, volume: 0.001 },
            { name: 'Martini Glass', description: 'Classic martini cocktail glass', weight: 0.28, dims: {length: 12, width: 12, height: 18}, volume: 0.003 },
            { name: 'Highball Glass', description: 'Tall highball tumbler', weight: 0.30, dims: {length: 7, width: 7, height: 15}, volume: 0.001 },
            { name: 'Crystal Decanter', description: 'Lead crystal wine decanter', weight: 1.2, dims: {length: 15, width: 15, height: 30}, volume: 0.007 },
        ],
        'Installation': [
            { name: 'Backdrop Frame 4x3m', description: 'Aluminum photo backdrop frame', weight: 28, dims: {length: 400, width: 10, height: 300}, volume: 1.200 },
            { name: 'Wedding Arch Floral', description: 'Decorative floral arch structure', weight: 22, dims: {length: 250, width: 80, height: 280}, volume: 5.600 },
            { name: 'Pipe and Drape System', description: '3m high drape system per 3m', weight: 15, dims: {length: 300, width: 10, height: 300}, volume: 0.900 },
            { name: 'Truss Structure 3m', description: 'Aluminum lighting truss section', weight: 32, dims: {length: 300, width: 30, height: 30}, volume: 0.270 },
            { name: 'Red Carpet Runner 10m', description: 'VIP red carpet runner', weight: 18, dims: {length: 1000, width: 120, height: 2}, volume: 0.240 },
        ],
        'Decor': [
            { name: 'Floral Centerpiece Luxury', description: 'Premium floral arrangement', weight: 3.5, dims: {length: 40, width: 40, height: 50}, volume: 0.080 },
            { name: 'LED Uplighter RGB', description: 'Wireless RGB uplight', weight: 2.8, dims: {length: 15, width: 15, height: 30}, volume: 0.007 },
            { name: 'Crystal Chandelier', description: 'Suspended crystal chandelier', weight: 45, dims: {length: 100, width: 100, height: 150}, volume: 1.500 },
            { name: 'Neon Sign Custom', description: 'Custom LED neon sign', weight: 5, dims: {length: 120, width: 10, height: 60}, volume: 0.072 },
            { name: 'Balloon Arch Kit', description: 'Complete balloon arch set', weight: 2, dims: {length: 300, width: 50, height: 250}, volume: 3.750 },
            { name: 'Mirror Ball 50cm', description: 'Disco mirror ball with motor', weight: 8, dims: {length: 50, width: 50, height: 50}, volume: 0.125 },
        ],
        'Lighting': [
            { name: 'Par LED Moving Head', description: 'Professional moving head light', weight: 12, dims: {length: 30, width: 30, height: 45}, volume: 0.041 },
            { name: 'Wash Light Bar', description: '4-head LED wash bar', weight: 8.5, dims: {length: 120, width: 15, height: 20}, volume: 0.036 },
            { name: 'Gobo Projector', description: 'Custom gobo projection light', weight: 6, dims: {length: 25, width: 25, height: 35}, volume: 0.022 },
            { name: 'Fairy Lights 20m', description: 'Warm white fairy light string', weight: 0.8, dims: {length: 2000, width: 5, height: 5}, volume: 0.050 },
        ],
    };
    ...
    for (const company of seededData.companies) {
        ...
        for (const [category, templates] of Object.entries(assetTemplates)) {
            for (const template of templates) {
                ...
                const brand = companyBrands.length > 0 ? (Math.random() > 0.3 ? randomItem(companyBrands) : null) : null;
                
                // Determine tracking method
                const trackingMethod = category === 'Glassware' || category === 'Decor' ? 'BATCH' : 'INDIVIDUAL';
                const totalQty = trackingMethod === 'BATCH' ? Math.floor(Math.random() * 100) + 20 : 1;
                ...
                const condition = conditionRand < 0.80 ? 'GREEN' : conditionRand < 0.95 ? 'ORANGE' : 'RED';
                ...
                if (condition === 'RED') {
                    ...
                    status = statusRand < 0.70 ? 'AVAILABLE' : statusRand < 0.90 ? 'BOOKED' : 'OUT';
                }
                ...
                if (condition !== 'GREEN') {
                    conditionHistory.push({
                        condition: condition,
                        notes: condition === 'ORANGE' ? 'Minor scratches on surface, functional' : 'Damaged during transport, needs repair',
                ...
                assets.push({
                    ...
                    condition: condition,
                    condition_notes: condition !== 'GREEN' ? (condition === 'ORANGE' ? 'Usable with minor wear' : 'Requires maintenance') : null,
                    refurb_days_estimate: condition === 'RED' ? Math.floor(Math.random() * 5) + 3 : condition === 'ORANGE' ? Math.floor(Math.random() * 2) + 1 : null,
                    condition_history: conditionHistory,
                    handling_tags: category === 'Glassware' ? ['Fragile', 'HighValue'] : 
                                   category === 'Furniture' && template.weight > 50 ? ['HeavyLift'] : 
                                   category === 'Installation' ? ['AssemblyRequired'] : [],
                    status: status,
                    last_scanned_at: status !== 'AVAILABLE' ? randomDate(new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), new Date()) : null,
                    last_scanned_by: status !== 'AVAILABLE' ? randomItem(seededData.users.filter(u => u.role !== 'CLIENT')).id : null,
...
async function seedCollections() {
    ...
    for (const company of seededData.companies) {
        ...
        const collectionTemplates = [
            { name: 'Executive Dinner Setup', description: 'Complete setup for executive dinners (10 pax)', category: 'Dining' },
            { name: 'Cocktail Reception Package', description: 'Full cocktail event setup', category: 'Cocktail' },
            { name: 'VIP Lounge Collection', description: 'Premium lounge furniture set', category: 'Lounge' },
            { name: 'Brand Activation Kit', description: 'Complete brand activation setup', category: 'Branding' },
        ];
        
        for (const template of collectionTemplates) {
            const brand = companyBrands.length > 0 && Math.random() > 0.5 ? randomItem(companyBrands) : null;
            
            // Generate collection images (hero images)
            const collectionName = `${template.name}${brand ? ` - ${brand.name}` : ''}`;
            const shortCollection = collectionName.length > 30 ? collectionName.substring(0, 28) + '..' : collectionName;
            const images = [
...
async function seedCollectionItems() {
    ...
    for (const collection of seededData.collections) {
        const companyAssets = seededData.assets.filter(a => a.company_id === collection.company_id);
        ...
        selectedAssets.forEach((asset, idx) => {
            collectionItems.push({
                ...
                asset: asset.id,
                default_quantity: asset.tracking_method === 'BATCH' ? Math.floor(Math.random() * 10) + 5 : 1,
...
async function seedOrders() {
    ...
    for (let i = 0; i < orderStatuses.length; i++) {
        ...
        const companyBrands = seededData.brands.filter(b => b.company_id === company.id);
        const brand = companyBrands.length > 0 && Math.random() > 0.3 ? randomItem(companyBrands) : null;
        ...
        const adjustedPrice = status !== 'DRAFT' && status !== 'SUBMITTED' ? basePrice * (0.95 + Math.random() * 0.15) : null;
        ...
        orders.push({
            ...
            brand_id: brand?.id || null,
            user_id: user.id,
            job_number: ['CONFIRMED', 'IN_PREPARATION', 'READY_FOR_DELIVERY', 'IN_TRANSIT', 'DELIVERED', 'AWAITING_RETURN', 'CLOSED'].includes(status) 
            ...
            venue_name: randomItem(['Dubai World Trade Centre', 'Atlantis The Palm', 'Burj Al Arab', 'Emirates Palace', 'Address Downtown', 'JW Marriott Marquis']),
            venue_location: {
                country: 'United Arab Emirates',
            ...
            special_instructions: Math.random() > 0.5 ? 'Please handle with care. Setup required by 6 PM.' : null,
            delivery_window: ['CONFIRMED', 'IN_PREPARATION', 'READY_FOR_DELIVERY', 'IN_TRANSIT', 'DELIVERED', 'AWAITING_RETURN', 'CLOSED'].includes(status)
                ? { start: new Date(eventStart.getTime() - 24 * 60 * 60 * 1000), end: new Date(eventStart.getTime() - 12 * 60 * 60 * 1000) }
                : null,
            pickup_window: ['AWAITING_RETURN', 'CLOSED'].includes(status)
                ? { start: new Date(eventEnd.getTime() + 12 * 60 * 60 * 1000), end: new Date(eventEnd.getTime() + 36 * 60 * 60 * 1000) }
                : null,
            calculated_totals: {
                volume: volume,
                weight: weight,
            ...
            transport_vehicle_type: parseFloat(volume) > 30 ? '10_TON' : parseFloat(volume) > 15 ? '7_TON' : 'STANDARD',
            tier_id: null, // DEPRECATED
            logistics_pricing: status !== 'DRAFT' && status !== 'SUBMITTED' ? {
                base_price: basePrice.toFixed(2),
                adjusted_price: adjustedPrice?.toFixed(2) || null,
                adjustment_reason: adjustedPrice && adjustedPrice !== basePrice ? 'Adjusted for complexity and special handling requirements' : null,
                adjusted_at: new Date(createdDate.getTime() + 24 * 60 * 60 * 1000),
                adjusted_by: seededData.users.find(u => u.role === 'LOGISTICS')?.id || null,
            } : null,
            platform_pricing: ['PENDING_APPROVAL', 'QUOTED', 'CONFIRMED', 'IN_PREPARATION', 'READY_FOR_DELIVERY', 'IN_TRANSIT', 'DELIVERED', 'AWAITING_RETURN', 'CLOSED'].includes(status) ? {
            ...
            final_pricing: ['QUOTED', 'CONFIRMED', 'IN_PREPARATION', 'READY_FOR_DELIVERY', 'IN_TRANSIT', 'DELIVERED', 'AWAITING_RETURN', 'CLOSED'].includes(status) ? {
            ...
            scanning_data: {},
            delivery_photos: ['IN_TRANSIT', 'DELIVERED', 'AWAITING_RETURN', 'CLOSED'].includes(status)
                ? [
...
async function seedAssetBookings() {
    ...
    for (const order of seededData.orders) {
        if (!['CONFIRMED', 'IN_PREPARATION', 'READY_FOR_DELIVERY', 'IN_TRANSIT', 'DELIVERED', 'AWAITING_RETURN'].includes(order.order_status)) {
            continue;
        }
    ...
    for (const order of seededData.orders) {
        ...
        if (['CLOSED'].includes(order.order_status)) {
            for (const item of orderItemsForOrder) {
                ...
                const returnCondition = Math.random() < 0.90 ? 'GREEN' : Math.random() < 0.5 ? 'ORANGE' : 'RED';
                ...
                scanEvents.push({
                    ...
                    notes: hasIssue ? (returnCondition === 'ORANGE' ? 'Minor wear and tear' : 'Significant damage, needs repair') : 'Returned in excellent condition',
                    photos: returnCondition === 'RED' ? [
...
async function seedOrderStatusHistory() {
    ...
    for (const order of seededData.orders) {
        ...
        statusProgression.forEach((status, idx) => {
            ...
            const timestamp = new Date(order.created_at.getTime() + daysOffset * 24 * 60 * 60 * 1000);
            const updatedBy = ['PRICING_REVIEW', 'IN_PREPARATION', 'READY_FOR_DELIVERY'].includes(status) 
...
async function seedInvoices() {
    ...
    for (const order of seededData.orders) {
        ...
        invoices.push({
            ...
            invoice_paid_at: order.financial_status === 'PAID' ? new Date(invoiceDate.getTime() + 7 * 24 * 60 * 60 * 1000) : null,
            payment_method: order.financial_status === 'PAID' ? randomItem(['Bank Transfer', 'Credit Card', 'Check']) : null,
            payment_reference: order.financial_status === 'PAID' ? `PAY-${Date.now()}-${Math.random().toString(36).substr(2, 6).toUpperCase()}` : null,
...
async function seedAssetConditionHistory() {
    ...
    for (const asset of seededData.assets) {
        ...
        for (let i = 0; i < historyEntries; i++) {
            ...
            const entryCondition = i === 0 ? asset.condition : (i === historyEntries - 1 ? 'GREEN' : randomItem(['GREEN', 'ORANGE']));
            
            history.push({
                platform_id: asset.platform_id,
                asset_id: asset.id,
                condition: entryCondition,
                notes: entryCondition === 'RED' ? 'Item damaged, sent to maintenance' :
                       entryCondition === 'ORANGE' ? 'Minor repairs completed' :
                       'Restored to working condition',
                photos: entryCondition === 'RED' ? [
...
async function seedNotificationLogs() {
    ...
    for (const order of seededData.orders) {
        const notificationTypes = getNotificationTypesForStatus(order.order_status, order.financial_status);
        const user = seededData.users.find(u => u.id === order.user_id);
        
        for (const notifType of notificationTypes) {
            const status = Math.random() < 0.90 ? 'SENT' : Math.random() < 0.5 ? 'FAILED' : 'QUEUED';
            ...
            notifications.push({
                ...
                sent_at: status === 'SENT' ? new Date(order.updated_at.getTime() + 60 * 60 * 1000) : null,
                message_id: status === 'SENT' ? `msg_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` : null,
...
function getStatusProgression(finalStatus: string): string[] {
    const progressions: Record<string, string[]> = {
        ...
        'CLOSED': ['DRAFT', 'SUBMITTED', 'PRICING_REVIEW', 'PENDING_APPROVAL', 'QUOTED', 'CONFIRMED', 'IN_PREPARATION', 'READY_FOR_DELIVERY', 'IN_TRANSIT', 'DELIVERED', 'AWAITING_RETURN', 'CLOSED'],
...
async function cleanupExistingData() {
    ...
    try {
        ...
        console.log('⚠️  Cleanup warning (tables may be empty):', error.message);
    }
}
...
async function main() {
    try {
        ...
        console.log(`  - Assets: ${seededData.assets.length} (${seededData.assets.length * 3} images)`);
        console.log(`  - Collections: ${seededData.collections.length} (${seededData.collections.length * 2} images)`);
        ...
        console.log(`  - Truck photos: ~${seededData.orders.filter(o => o.delivery_photos.length > 0).length * 2}`);
        console.log(`  - Damage photos: Generated for RED condition items`);
        console.log(`  - Total placeholder images: ${seededData.assets.length * 3 + seededData.collections.length * 2 + seededData.brands.length + 50}+`);

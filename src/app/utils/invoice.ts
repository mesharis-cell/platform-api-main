...
import { db } from "../../db";
import { companies, invoices, orders, serviceRequests } from "../../db/schema";
...
export const serviceRequestInvoiceGenerator = async (
    ...
    const [invoice] = await db
        ...
        .where(
            and(
                eq(invoices.service_request_id, serviceRequestId),
                eq(invoices.platform_id, platformId)
            )
        );
    ...
    if (invoice && !regenerate) {
        throw new Error(
            "Invoice already exists for this service request. Use regenerate flag to create a new PDF."
        );
    }
    if (regenerate && invoice && invoice.invoice_paid_at) {
        throw new Error("Cannot regenerate invoice after payment has been confirmed");
    ...
    if (!serviceRequest) {
        throw new CustomizedError(
            ...
            "Service request not found to generate invoice"
        );
    }
    ...
    const key = `invoices/service-request/${company?.name.replace(/\s/g, "-").toLowerCase() || "unknown-company"}/${invoiceNumber}.pdf`;
    const pdfUrl = await uploadPDFToS3(pdfBuffer, invoiceNumber, key);

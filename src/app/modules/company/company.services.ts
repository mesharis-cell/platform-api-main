...
const getCompanies = async (query: Record<string, any>, platformId: string) => {
    const { search_term, page, limit, sort_by, sort_order, include_inactive, include_deleted } =
        query;

    // Step 1: Validate query parameters
    if (sort_by) queryValidator(companyQueryValidationConfig, "sort_by", sort_by);
    if (sort_order) queryValidator(companyQueryValidationConfig, "sort_order", sort_order);
...
const updateCompany = async (id: string, data: any, platformId: string, user: AuthUser) => {
    try {
        ...
        if (
            ...
            !(
                user.permissions.includes(PERMISSIONS.COMPANIES_SET_MARGIN) ||
                user.permissions.includes(PERMISSIONS.COMPANIES_ALL)
            )
        ) {
            throw new CustomizedError(
                httpStatus.UNAUTHORIZED,
                "You are not authorized to update platform margin percentage"
            );
        }
...
const deleteCompany = async (id: string, platformId: string) => {
    ...
    const conditions: any[] = [eq(companies.id, id), eq(companies.platform_id, platformId)];

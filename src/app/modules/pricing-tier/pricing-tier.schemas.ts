...
const updatePricingTierSchema = z.object({
    body: z
        .object({
            volume_min: z.number().min(0, "Minimum volume must be at least 0").optional(),
            volume_max: z
                .number()
                .min(0, "Maximum volume must be at least 0")
                .optional()
                .nullable(),
            base_price: z.number().min(0, "Base price must be at least 0").optional(),
        ...
        .refine(
            ...
            {
                message:
                    "Country and city cannot be updated. Please create a new pricing tier instead.",
                path: ["country"],
            }
        ),
});

...
const updatePriceForTransport = async (
    ...
    const [order] = await db.select().from(orders).where(eq(orders.order_pricing_id, priceRecord.id)).limit(1);
    ...
    const logisticsSubTotal = baseOpsTotal + transport_rate + Number((priceRecord.line_items as any).catalog_total || 0);
    const marginAmount = logisticsSubTotal * (Number((priceRecord.margin as any).percent) / 100);
    const finalTotal = logisticsSubTotal + marginAmount + Number((priceRecord.line_items as any).custom_total || 0);
    ...
    const [updatedPrice] = await db.update(prices).set(pricingDetails).where(eq(prices.id, id)).returning();

...
const createBrand = async (data: CreateBrandPayload) => {
    try {
        ...
        const [company] = await db
            .select()
            .from(companies)
            .where(and(eq(companies.id, data.company_id), isNull(companies.deleted_at)));
...
const getBrands = async (query: Record<string, any>, user: AuthUser, platformId: string) => {
    const { search_term, page, limit, sort_by, sort_order, company_id, include_inactive } = query;

    // Step 1: Validate query parameters
    if (sort_by) queryValidator(brandQueryValidationConfig, "sort_by", sort_by);
    if (sort_order) queryValidator(brandQueryValidationConfig, "sort_order", sort_order);
    ...
    if (search_term) {
        conditions.push(ilike(brands.name, `%${search_term.trim()}%`));
    }
...
const getBrandById = async (id: string, user: AuthUser, platformId: string) => {
    ...
    const conditions: any[] = [eq(brands.id, id), eq(brands.platform_id, platformId)];

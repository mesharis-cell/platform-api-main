...
const exportAssetCatalogService = async (
    ...
    const includePhotos = query.include_photos === "true" || query.include_photos === "1";

    const conditions: any[] = [eq(assets.platform_id, platformId), isNull(assets.deleted_at)];
    if (query.company_id) conditions.push(eq(assets.company_id, query.company_id));
    ...
    if (includePhotos) {
        ...
        return { format: "xlsx", buffer, companyName };
    }

    const csvRows = generateAssetCatalogCsvRows(rows);
    return { format: "csv", csv: Papa.unparse(csvRows), companyName };
};
